...
- (Local CLI): heroku domains:add api.bookskye.co.uk --app book-skye-{env} #add custom domain to heroku dyno
- (Local CLI): heroku certs:auto:enable --app book-skye-{env} #add ssl certs to custom domain (even if hosted elsewhere)
- (Local CLI): heroku domains --app book-skye-{env} -> #Provides DNS target for Route 53
- (Browser: vercel.com)
...
```bash
git clone git@github-book-skye:bookskye/monorepo.git
```
...
heroku info --app book-skye-{env} #Url
heroku logs --tail --app book-skye-{env} #Live logs
heroku logs --num 200 --app book-skye-{env} #Most recent lines
heroku config --app book-skye-{env} #Env vars
heroku run sh --app book-skye-{env} #Bash inside docker container

## Installation

`nvm use 24` #Compatible with: Node 24.13.1
`pnpm install` @ root only
Copy apps/book-skye-api/.local.env.example -> .local.env
Copy apps/book-skye-api/.e2e.env.example -> .e2e.env
...
pnpm db
pnpm db-down
pnpm --filter=book-skye-api dev
pnpm --filter=book-skye-guest-website dev
pnpm --filter=skye-glamping-website dev
pnpm --filter=book-skye-host-app dev # Supports native modules
...
pnpm --filter=book-skye-api lint

# Migration - Generate
pnpm --filter=book-skye-api migration:generate src/migrations/name
